...
<section id="tentang-kami" class="py-16 md:py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div>
                <p class="text-hijau-600 font-semibold text-sm uppercase tracking-wider mb-3">Tentang Kami</p>
                <h2 class="text-3xl md:text-4xl font-bold text-gray-900 mb-6 leading-tight">
                    Mengelola Informasi Secara <span class="text-hijau-600">Transparan</span>
                </h2>
                <p class="text-gray-600 leading-relaxed mb-8">
                    Kami berkomitmen untuk mengelola informasi dan dokumentasi secara transparan demi peningkatan pelayanan publik di IAIN Bone. Sesuai amanat UU Keterbukaan Informasi Publik.
                </p>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
                @php
                    $menuItems = [
                        ['icon' => 'fa-handshake', 'title' => 'Visi & Misi', 'url' => '/tentang-kami/visi-misi'],
                        ['icon' => 'fa-tasks', 'title' => 'Tugas & Fungsi', 'url' => '/tentang-kami/tugas-fungsi'],
                        ['icon' => 'fa-crown', 'title' => 'Profil Pejabat', 'url' => '/tentang-kami/profil-pejabat'],
                        ['icon' => 'fa-sitemap', 'title' => 'Struktur Organisasi', 'url' => '/tentang-kami/struktur-organisasi'],
                        ['icon' => 'fa-puzzle-piece', 'title' => 'Profil Bidang', 'url' => route('bidang-sektoral.index')],
                    ];
                @endphp
...
<section class="py-12 md:py-16 bg-gray-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-6 md:mb-8">
            <p class="text-hijau-600 font-semibold text-sm uppercase tracking-wider mb-2">Pencarian</p>
            <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Temukan Informasi</h2>
        </div>
        <form action="{{ route('search') }}" method="GET" class="flex">
            <div class="flex-1 flex items-center bg-white border border-r-0 border-gray-300 rounded-l-xl px-4 focus-within:ring-2 focus-within:ring-hijau-500 focus-within:border-hijau-500 transition">
                <i class="fa-solid fa-magnifying-glass text-gray-400 mr-3 text-sm"></i>
                <input type="search" name="q" placeholder="Cari informasi..." value="{{ request('q') }}"
                       class="w-full py-3 text-sm placeholder-gray-400 focus:outline-none border-0 bg-transparent">
            </div>
            <button type="submit" class="px-6 py-3 bg-hijau-600 hover:bg-hijau-700 text-white rounded-r-xl transition text-sm font-medium">
                Cari
            </button>
        </form>
    </div>
...
<section class="py-16 md:py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 md:mb-14">
            <p class="text-hijau-600 font-semibold text-sm uppercase tracking-wider mb-2">Terbaru</p>
            <h2 class="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900">Informasi & Publikasi</h2>
        </div>
        ...
        <div class="grid lg:grid-cols-2 gap-6 md:gap-8">
            ...
            <div class="bg-gray-50 rounded-2xl border border-gray-200 p-5 md:p-6 flex flex-col">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg md:text-xl font-bold text-gray-800 flex items-center gap-2">
                        <span class="w-8 h-1 bg-hijau-600 rounded-full"></span> Berita Terbaru
                    </h3>
                    <a href="{{ route('berita.index') }}" class="text-sm text-hijau-600 hover:text-hijau-700 font-medium">Lihat Semua</a>
                </div>
                ...
                <div class="flex-1 divide-y divide-gray-200">
                    ...
                    <a href="{{ route('berita.show', $post->slug) }}" class="flex items-center justify-between py-3.5 first:pt-0 last:pb-0 group">
                        <span class="text-gray-700 group-hover:text-hijau-700 transition text-sm pr-4 line-clamp-2">{{ $post->title }}</span>
                        <i class="fas fa-chevron-right text-xs text-gray-300 group-hover:text-hijau-500 transition shrink-0"></i>
                    </a>
                    @empty
                    <p class="text-sm text-gray-400 py-4">Belum ada berita terbaru.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-2xl border border-gray-200 p-5 md:p-6 flex flex-col">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg md:text-xl font-bold text-gray-800 flex items-center gap-2">
                        <span class="w-8 h-1 bg-hijau-600 rounded-full"></span> Publikasi Resmi
                    </h3>
                    <a href="{{ route('publikasi.index') }}" class="text-sm text-hijau-600 hover:text-hijau-700 font-medium">Lihat Semua</a>
                </div>
                
                <div class="flex-1 divide-y divide-gray-200">
                    @forelse($dokumen as $doc)
                    <a href="{{ route('publikasi.show', $doc->slug) }}" class="flex items-center justify-between py-3.5 first:pt-0 last:pb-0 group">
                        <span class="text-gray-700 group-hover:text-hijau-700 transition text-sm pr-4 line-clamp-2">{{ $doc->judul }}</span>
                        <i class="fas fa-chevron-right text-xs text-gray-300 group-hover:text-hijau-500 transition shrink-0"></i>
                    </a>
                    @empty
                    <p class="text-sm text-gray-400 py-4">Belum ada publikasi terbaru.</p>
                    @endforelse
                </div>
            </div>
        </div>
...
<section id="layanan" class="py-16 md:py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 md:mb-14">
            <p class="text-hijau-600 font-semibold text-sm uppercase tracking-wider mb-2">Layanan Kami</p>
            <h2 class="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900">Pusat Layanan & Pengaduan</h2>
        </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <a href="https://www.lapor.go.id/" target="_blank" class="group p-5 md:p-6 border border-gray-200 rounded-2xl hover:border-red-300 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center">
                ...
                <h5 class="text-base md:text-lg font-bold text-gray-800 mb-1.5">Pengaduan Masyarakat</h5>
                <p class="text-xs md:text-sm text-gray-500">Kanal resmi pengaduan (SP4N-LAPOR!)</p>
            </a>
            ...
            <a href="{{ route('layanan-pengaduan.faq-umum') }}" class="group p-5 md:p-6 border border-gray-200 rounded-2xl hover:border-green-300 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center">
                ...
                <h5 class="text-base md:text-lg font-bold text-gray-800 mb-1.5">Pertanyaan Umum</h5>
                <p class="text-xs md:text-sm text-gray-500">Jawaban atas pertanyaan yang sering diajukan</p>
            </a>
            ...
            <a href="{{ route('layanan-pengaduan.daftar-layanan') }}" class="group p-5 md:p-6 border border-gray-200 rounded-2xl hover:border-blue-300 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center">
                ...
                <h5 class="text-base md:text-lg font-bold text-gray-800 mb-1.5">Daftar Layanan</h5>
                <p class="text-xs md:text-sm text-gray-500">Informasi mengenai berbagai layanan kami</p>
            </a>
            ...
            <a href="{{ route('layanan-pengaduan.cek-status') }}" class="group p-5 md:p-6 border border-gray-200 rounded-2xl hover:border-amber-300 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center">
                ...
                <h5 class="text-base md:text-lg font-bold text-gray-800 mb-1.5">Cek Status</h5>
                <p class="text-xs md:text-sm text-gray-500">Lacak status permohonan atau pengaduan Anda</p>
            </a>
        </div>
    </div>
</section>
...
<section class="relative py-20 md:py-24 bg-hijau-800 overflow-hidden">
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-6">Ada Pertanyaan Terkait Informasi?</h2>
        <p class="text-base md:text-lg text-hijau-200 mb-8 md:mb-10 max-w-2xl mx-auto">Jangan ragu untuk menghubungi tim PPID kami. Kami siap membantu Anda mendapatkan informasi yang dibutuhkan.</p>
